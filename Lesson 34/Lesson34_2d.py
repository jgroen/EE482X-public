...
if True:
    ...
    plt.arrow(0, float(np.mean(pos[:, 1])), np.sin(theta) * 0.03, np.cos(theta) * 0.03, head_width=0.005,
              head_length=0.005, fc='r', ec='r')
    plt.text(0.018, 0.07, 'theta (20 deg)', color='red')
    plt.savefig('2d_beamforming_ula.png', bbox_inches='tight')
    plt.show()
...
if True:
    ...
    plt.grid()
    plt.gca().set_aspect('equal', adjustable='box')
    plt.savefig('2d_beamforming_element_pos.png', bbox_inches='tight')
    plt.show()
...
if True:
    ...
    plt.imshow(results.T, extent=(theta_scan[0] * 180 / np.pi, theta_scan[-1] * 180 / np.pi, phi_scan[0] * 180 / np.pi,
                                  phi_scan[-1] * 180 / np.pi), origin='lower', aspect='auto', cmap='viridis')
    ...
    plt.xlabel('Azimuth angle [degrees]')
    plt.ylabel('Elevation angle [degrees]')
    plt.savefig('2d_beamforming_2dplot.png', bbox_inches='tight')
    plt.show()
...
if True:
    ...
    results = np.maximum(results, 1e-12)
    results = 10 * np.log10(results)  # convert to dB
    results[results < -5] = -5 # crop the z axis to some level of dB
    ...
    fig.savefig('2d_beamforming_3dplot.png', bbox_inches='tight')
    plt.show()
