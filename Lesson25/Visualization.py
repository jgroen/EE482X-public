...
def plot_constellations(samples_rx, recovered_symbols):

    plt.figure(figsize=(10,4))

    # Before timing recovery (raw RX samples)
    plt.subplot(1,2,1)
    plt.scatter(
        np.real(samples_rx[60:-60]),
        np.imag(samples_rx[60:-60]),
        s=5
    )
    plt.title("Constellation Before Timing Recovery")
    plt.xlabel("I")
    plt.ylabel("Q")
    plt.grid(True)

    # After timing recovery (symbol decisions)
    plt.subplot(1,2,2)
    plt.scatter(
        np.real(recovered_symbols[8:]),
        np.imag(recovered_symbols[8:]),
        s=10
    )
...
def interactive_mu_visualization(samples_rx, sps):

    fig, ax = plt.subplots()
    plt.subplots_adjust(bottom=0.25)

    t = np.arange(len(samples_rx))

    # Interpolator (key fix)
    interp_real = interp1d(t, np.real(samples_rx), kind='cubic', fill_value="extrapolate")

    mu_init = 0.0

    def get_samples(mu):
        sample_times = np.arange(0, len(samples_rx)/sps) * sps + mu * sps
        sample_times = sample_times[sample_times < len(samples_rx)]
        return sample_times, interp_real(sample_times)

    times, samp = get_samples(mu_init)

    line, = ax.plot(t, np.real(samples_rx), label="Signal")
    scatter = ax.scatter(times, samp, color='red', label="Samples")

    ax.set_title("Effect of Timing Offset (mu)")
    ax.legend()

    # Slider
    ax_mu = plt.axes([0.2, 0.1, 0.6, 0.03])
    slider = Slider(ax_mu, 'mu', 0, 1, valinit=0)

    def update(val):
        mu = slider.val
        times, samp = get_samples(mu)

        scatter.set_offsets(np.c_[times, samp])
        fig.canvas.draw_idle()

    slider.on_changed(update)

    plt.show()
def plot_psd(samples, fs=1.0):
    ...
    plt.figure()
    plt.plot(np.fft.fftshift(f), np.fft.fftshift(10*np.log10(Pxx + 1e-12)))
...
def main():
    ...
    plot_constellations(freq_offset_samples, recovered_symbols)
    interactive_mu_visualization(freq_offset_samples, sps)
    plot_psd(freq_offset_samples, fs=1e6)
