...
# FIX: use np.cov like PySDR
R_training = np.cov(X_A_B)
...
s = s.reshape(-1,1)
w = (Rinv_training @ s)/(s.conj().T @ Rinv_training @ s)

# Calc beam pattern (unchanged)
w = w.squeeze()
N_fft = 2048
...
# Plot beam pattern and DOA results (unchanged)
plt.plot(theta_bins * 180 / np.pi, w_fft_dB)
plt.plot(theta_scan * 180 / np.pi, results, 'r')
plt.vlines(ymax=np.max(results), ymin=np.min(results),
           x=max_angle*180/np.pi, color='g', linestyle='--')
